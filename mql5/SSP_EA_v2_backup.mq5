...
input string   SignalFile      = "indicator5_signals.json";
...
void ProcessSignal(const string &signal_json)
{
   ...
   // Validate signal
   if(direction == "" || sl_distance <= 0 || tp_rr_ratio <= 0 || ref_price <= 0)
   {
      Print("[IND5] Invalid signal: missing required fields");
      return;
   }

   // Check for duplicate (already processed this signal)
...
double CalculateLotSize(double entry_price, double sl_price)
{
   ...
   // Enforce minimum lot (match backtest.py logic with max())
   if(lot_size < minlot)
      lot_size = minlot;
   ...
   // Normalize to 2 decimal places (standard lot format)
